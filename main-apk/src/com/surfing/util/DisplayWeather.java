...
public class DisplayWeather
{
    ...
    public static void updateWeatherDisplay(final Context context, final Activity activity)
    {
       ...
       /*
        ...
        View view = activity.findViewById(R.id.weather_display_id);
        Log.i("DisplayWeather","view = "+view);
        if(view == null){
            return;
        }
        TextView mCityText = (TextView) activity.findViewById(R.id.city_text_id);
        TextView mTempCurrent = (TextView) activity.findViewById(R.id.temp_current_id);
        TextView mConditionText = (TextView) activity.findViewById(R.id.condition_current_id);
        ImageView mConditionImg = (ImageView) activity.findViewById(R.id.condition_icon_id);
        mCityText.setText(DisplayWeather.getCurrentCity(context));
        if (mWeatherInfo != null) 
        {
            //Log.i("weather", "getmWeatherCurrent().getmTemp_c() == " + mWeatherInfo.getmWeatherCurrent().getmTemp_c());
            //Log.i("weather", "getmWeatherCurrent().getmCondition() == " + mWeatherInfo.getmWeatherCurrent().getmCondition());
            if(mWeatherInfo.getmWeatherCurrent() != null){
                mTempCurrent.setText("" + mWeatherInfo.getmWeatherCurrent().getmTemp_c() +context.getString(R.string.degree_text));
                Log.i("weather", "getmWeatherCurrent().getmCondition() == " + mWeatherInfo.getmWeatherCurrent().getmCondition());
                mConditionText.setText(mWeatherInfo.getmWeatherCurrent().getmCondition());
                ImageDownloader imgDown = new ImageDownloader();
                String url = "http://g0.gstatic.com" + mWeatherInfo.getmWeatherCurrent().getmIconUrl();
                imgDown.download(url, mConditionImg, R.drawable.notify_online,context);
            }
        }
